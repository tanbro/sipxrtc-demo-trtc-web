...
function login() {
  ...
  if (!navigator.cookieEnabled) {
    alert("由于您的浏览器环境不支持 Cookie，无法继续执行演示!");
    return;
  }
  ///
  const btn = $("#login-btn");
    ...
    .then((response) => {
      if (!response.ok) {
        ...
        response.text().then((txt) => {
          alert(
            `申请 TRTC 进房参数失败: (${response.status}) ${response.statusText}\r\n${txt}`
          );
        });
        return;
      }
      response
        ...
        .then((value) => {
          // 在这里 TRTC 进房 !
          console.log("收到 TRTC 进房参数: ", value);
          trtcRoomOptions = value;
          rtc = new RtcClient(trtcRoomOptions);
          join();
          // 关闭摄像头???
          $("#video-btn").attr("src", "./img/big-camera-off.png");
          $("#member-me")
...
function join() {
  ...
  $("#login-root").hide();
  $("#room-root").show();
  $("#header-phoneNumber").html("手机号: " + phoneNumber);
  $("#member-me")
    .find(".member-id")
    .html(trtcRoomOptions.userId + "(我)");
  /**
   * Add-by: liuxy
   *
   * 调用 make_call
...
function setBtnClickFuc() {
    ...
    fetch(url, {
      ...
      .then((response) => {
        if (!response.ok) {
          response.text().then((txt) => {
            alert(
              `通知服务器退出 TRTC 房间失败: (${response.status}) ${response.statusText}\r\n${txt}`
            );
          });
          return;
        }
   ...
   * 获取 SMS 验证码
   */
  $("#btnValidateSms").on("click", () => {
    const btn = $("#btnValidateSms");
    if (btn.hasClass("disabled")) {
      return;
    }
    if (!$("#phoneNumber")[0].reportValidity()) {
      return;
    }
    ...
    fetch(url, {
      ...
      .then((response) => {
        if (!response.ok) {
          ...
          response.text().then((txt) => {
            alert(
              `发送短信验证码失败: (${response.status}) ${response.statusText}\r\n${txt}`
            );
          });
          return;
        }
        console.log("短信验证码已经发送了.");
        // 开始允许发送倒数
        response
          .json()
          .then((data) => {
            let c = data.sendTimeout;
            btn.addClass("disabled");
            let setBtnCaption = () => btn.val(`短信验证码已发送(${c})`);
            setBtnCaption();
