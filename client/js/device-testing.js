...
const deviceFailAttention =
  ...
  '3. 检查系统设置，允许浏览器访问麦克风<br>' +
  '4. 检查浏览器设置，允许网页访问麦克风<br>' +
  '5. 检查麦克风是否正确连接并开启<br>' +
  '6. 尝试重新连接麦克风<br>' +
  '7. 尝试重启设备后重新检测';
...
async function deviceTestingInit() {
  ...
  */
  ...
  $('#voice-testing, #mic-testing, #network-testing').on('click', function() {
...
function deviceDialogInit() {
  if (!localStorage.getItem('txy_device_testing')) {
    ...
    let showDeviceStatus = function() {
      ...
      if (!(hasVoiceConnect && hasMicConnect && hasNetworkConnect)) {
...
*/	
  if (!(hasMicDevice && hasVoiceDevice)) {
    connectInfo = `未检测到${hasVoiceDevice ? '' : '【扬声器】'}${
        hasMicDevice ? '' : '【麦克风】'}设备，请检查设备连接`;
    return connectInfo;
  }
...
async function startDeviceConnect() {
  ...
  let showDeviceConnectInfo = function() {
    ...
    $('#device-voice, #device-mic, #device-network').removeClass(
    ...
    setTimeout(() => {
	  ...
	  */
      ...
      /*const connectResult = hasCameraConnect && hasVoiceConnect && hasMicConnect && hasNetworkConnect;*/
      const connectResult = hasVoiceConnect && hasMicConnect && hasNetworkConnect;
      if (connectResult) {
        $('#connect-info')
...
async function testDownlinkNetworkQuality() {
  ...
  window.downlinkClient.on('stream-added', async event => {
    await window.downlinkClient.subscribe(event.stream, { audio: true, video: false });
...
/*  ['camera', 'voice', 'mic', 'network'].forEach(item => {*/
  ['voice', 'mic', 'network'].forEach(item => {
    $(`#${item}-testing`)
...
function handleGetUserMediaError(error) {
  switch (error.name) {
    case 'NotReadableError':
      ...
      alert(
        '暂时无法访问麦克风，请确保系统授予当前浏览器麦克风权限，并且没有其他应用占用摄像头/麦克风'
      );
      return;
    case 'NotAllowedError':
      alert('用户/系统已拒绝授权访问麦克风');
      return;
    case 'NotFoundError':
      // 找不到摄像头或麦克风设备
      alert('找不到麦克风设备');
